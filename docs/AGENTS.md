...
- Never paste agent-facing system prompts here. Those live in [`src/agents/`](../packages/omo-opencode/src/agents/) or [`src/features/builtin-skills/`](../packages/omo-opencode/src/features/builtin-skills/).
- Never document changing config keys without also updating [`src/config/schema/`](../packages/omo-opencode/src/config/schema/) and re-running `bun run build:schema`.
