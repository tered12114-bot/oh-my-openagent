...
Loaded alongside `.opencode/` by [`src/features/opencode-skill-loader/`](../packages/omo-opencode/src/features/opencode-skill-loader). When both directories declare the same skill or command name, the higher-priority scope wins per the loader's deduplication rules.
